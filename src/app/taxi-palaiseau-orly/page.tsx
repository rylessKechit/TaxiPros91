...
const ROUTE = popularRoutes.find(r => r.slug === 'taxi-palaiseau-orly')!
...
const faqs = [
  {
    ...
    a: `Le prix d'un taxi de ${FROM} vers l'aéroport d'Orly est compris entre ${PRICE_MIN}€ et ${PRICE_MAX}€ selon l'heure (tarif jour ou tarif nuit/dimanche/jours fériés) et les conditions de circulation. Ce tarif est fixe, communiqué à la réservation et inclut la prise en charge à votre adresse (centre-ville, Lozère, Le Pileu, plateau de Saclay), l'attente au terminal et l'aide aux bagages.`,
  },
  {
    q: `Combien de temps dure le trajet ${FROM} Orly en taxi ?`,
    a: `Le trajet ${FROM} → Aéroport d'Orly dure en moyenne ${DURATION} minutes en taxi via la RN118 puis l'A86 et l'A106. Aux heures de pointe (7h-9h et 17h-20h), comptez 25 à 35 minutes en raison du trafic sur la N118. En heures creuses ou la nuit, le trajet peut se faire en 15 à 18 minutes seulement.`,
  },
  {
    q: `Comment réserver un taxi de ${FROM} vers Orly ?`,
    a: `Trois options : appelez le ${SITE.phone.display} disponible 24h/24, utilisez le formulaire de réservation en ligne (confirmation immédiate par email et SMS), ou réservez à l'avance pour un trajet matinal. Pour un vol tôt le matin depuis Palaiseau, nous recommandons de réserver au moins la veille (J-1).`,
  },
  {
    q: `Desservez-vous tous les quartiers de ${FROM} ?`,
    a: `Oui, nos taxis interviennent dans l'ensemble de Palaiseau : centre-ville, Lozère, Le Pileu, quartier des Garennes, ainsi que dans toutes les zones professionnelles environnantes (École polytechnique, plateau de Saclay, ZA Hélène Boucher). Précisez votre adresse exacte à la réservation.`,
  },
  {
    q: 'Quels moyens de paiement acceptez-vous ?',
    a: `Tous nos taxis acceptent la carte bancaire (Visa, Mastercard, American Express), les espèces et les bons de transport CPAM. La facturation entreprise est disponible sur demande pour vos déplacements professionnels depuis Palaiseau vers Orly (utile pour les entreprises de Saclay et de Polytechnique).`,
  },
  {
    q: `Proposez-vous l'aller-retour ${FROM} ↔ Orly ?`,
    a: `Oui, nous proposons un forfait aller-retour ${FROM} ↔ Orly à tarif préférentiel. Idéal pour un voyage court ou un déplacement professionnel. Indiquez les horaires aller et retour à la réservation ; le chauffeur vous attend à votre vol retour avec suivi en temps réel.`,
  },
  {
  ...
  },
  {
    ...
    a: `Les bagages standards (valises, sacs) sont inclus dans le tarif. Pour les groupes (jusqu'à 7 passagers) ou les bagages volumineux (skis, instruments, poussette), nous proposons notre service Van avec un léger supplément. Précisez votre besoin à la réservation.`,
  },
]
...
const usp = [
  ...
  { icon: Shield, title: 'Chauffeurs locaux', desc: 'Conducteurs basés à Palaiseau, qui connaissent parfaitement les itinéraires alternatifs et les zones de Saclay.' },
]

const avis = [
  { nom: 'Camille R. (Palaiseau centre)', note: 5, texte: `Réservé la veille pour un vol à 6h45 depuis Orly 4. Le chauffeur était à mon adresse rue Charles de Gaulle à 4h45 pétantes, trajet fluide par la N118, tarif annoncé respecté. Aucun stress, je referai appel.`, date: 'Mars 2025' },
  { nom: 'David M. (Lozère)', note: 5, texte: `Je travaille à Polytechnique et je prends régulièrement TAXI Pro 91 pour aller à Orly. Toujours ponctuel, voiture impeccable, et le suivi de vol au retour est vraiment fonctionnel.`, date: 'Février 2025' },
  { nom: 'Hélène B. (Le Pileu)', note: 5, texte: `Aller-retour Palaiseau-Orly pour un week-end à Lisbonne. Prix très correct, chauffeur sympa, dépose directement devant Orly 1. Recommandé.`, date: 'Janvier 2025' },
  { nom: 'Olivier T. (plateau de Saclay)', note: 5, texte: `Vol retour décalé d'1h30, le chauffeur a attendu sans broncher et sans supplément. Service de qualité, je préfère largement à un VTC à prix variable.`, date: 'Avril 2025' },
]

const autresTrajets = popularRoutes.filter(r => r.fromSlug === 'palaiseau' && r.slug !== 'taxi-palaiseau-orly')

export default function TaxiPalaiseauOrly() {
  return (
    <>
      <Header />
      ...
      <ServiceSchema
        name={`Taxi ${FROM} ${TO}`}
        description={`Service de taxi entre ${FROM} (91120) et l'aéroport d'Orly. Tarif fixe dès ${PRICE_MIN}€, trajet en ${DURATION} minutes via RN118 et A106, disponible 24h/24 avec suivi de vol inclus.`}
        url={URL}
      ...
      <main>
            ...
            <div className="grid lg:grid-cols-2 gap-10 items-start">
              <div>
                <div className="flex items-center gap-2 text-yellow-600 mb-3">
                  ...
                  <span className="font-medium">{FROM} (91120) → Aéroport Orly</span>
                </div>
                ...
                <p className="text-lg text-gray-600 mb-6 leading-relaxed">
                  ...
                  Trajet en <strong>{DURATION} minutes</strong> via la RN118 puis l'A86 et l'A106, prise en charge à votre adresse
                  à {FROM} (centre, Lozère, Le Pileu) et dépose directe à votre terminal (Orly 1, 2, 3 ou 4). Service 24h/24 et 7j/7.
                </p>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              ...
              <p className="text-gray-600 mb-8">
                ...
                aucun compteur qui s'emballe dans les bouchons de la N118. Voici notre grille indicative pour le trajet {FROM} Orly,
                applicable depuis tous les quartiers de Palaiseau (centre-ville, Lozère, Le Pileu, plateau de Saclay).
              </p>
              ...
              <div className="mt-8 p-5 bg-yellow-50 border border-yellow-200 rounded-xl">
                <p className="text-gray-900 text-sm leading-relaxed">
                  ...
                  négociés pour vos trajets Palaiseau-Orly — souvent plus avantageux et plus rapides pour les habitants du sud francilien.
                </p>
              </div>
            </div>
          </div>
          ...
          <div className="container mx-auto px-4">
            <div className="max-w-4xl mx-auto">
              ...
              <p className="text-gray-600 mb-6">
                Le trajet entre {FROM} et l'aéroport d'Orly couvre environ <strong>18 kilomètres</strong> et
                se parcourt en moyenne en <strong>{DURATION} minutes</strong>. Palaiseau est idéalement située pour rejoindre
                rapidement Orly grâce à la <strong>RN118</strong>, un axe rapide qui dessert directement l'A86 puis l'A106.
                Un atout majeur pour les voyageurs qui veulent éviter le stress de la route ou les correspondances RER.
              </p>
              ...
              <div className="grid md:grid-cols-2 gap-6 mb-8">
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  ...
                  <p className="text-gray-600 text-sm leading-relaxed">
                    L'itinéraire le plus direct emprunte la <strong>RN118</strong> en direction de Paris,
                    puis bifurque sur l'<strong>A86</strong> à hauteur de Vélizy/Châtenay, avant de rejoindre
                ...
                <div className="bg-white rounded-xl p-6 border border-gray-200">
                  ...
                  <ul className="text-gray-600 text-sm space-y-2">
                    <li><strong>Heures creuses :</strong> 15-20 minutes</li>
                    <li><strong>Heures de pointe (7h-9h / 17h-20h) :</strong> 25-35 minutes</li>
                    <li><strong>Nuit (après 22h) :</strong> 15 minutes</li>
                    <li><strong>Week-end :</strong> 18-22 minutes</li>
                  </ul>
                </div>
              </div>

              <p className="text-gray-600 mb-6">
                <strong>Conseil de pro :</strong> pour un vol au départ d'Orly, partez de {FROM} <strong>2h15 avant</strong> votre
                vol en semaine aux heures de pointe, <strong>1h30 avant</strong> en heures creuses. Nos chauffeurs
                connaissent les itinéraires alternatifs (par la D444 ou la N20 via Massy-Chilly) en cas d'incident sur la N118 ou l'A86.
              </p>

              <p className="text-gray-600">
                Les habitants des quartiers excentrés comme le <strong>plateau de Saclay</strong>, le <strong>Pileu</strong> ou
                la <strong>cité scolaire de Lozère</strong> bénéficient d'un accès direct à la RN118 sans détour : prévoyez simplement
                5 minutes supplémentaires si vous partez d'une adresse dans ces secteurs. Pour l'École polytechnique et l'IOGS,
                nous appliquons le même tarif fixe (le campus est à 4 minutes de la N118).
              </p>
            </div>
          </div>
        </section>
        ...
        <section className="py-12 md:py-16 bg-gray-50">
          <div className="container mx-auto px-4">
            ...
            <p className="text-gray-600 text-center mb-10 max-w-2xl mx-auto">
              Plus de 10 ans d'expérience sur l'axe Palaiseau-Saclay-Orly, des centaines de transferts aéroport chaque mois,
              avec une connaissance fine des entreprises du plateau (Polytechnique, EDF Lab, Thales, CEA Saclay).
            </p>
          ...
          <div className="container mx-auto px-4">
            ...
            <div className="grid md:grid-cols-3 gap-6 max-w-5xl mx-auto">
              ...
              <Link href="/taxi-essonne-disneyland" className="bg-gray-50 rounded-xl p-6 border border-gray-200 hover:border-yellow-400 transition-colors block">
                ...
                <h3 className="font-bold text-gray-900 text-lg mb-2">Palaiseau → Disneyland Paris</h3>
                <p className="text-gray-600 text-sm mb-3">Transfert direct depuis Palaiseau vers Marne-la-Vallée.</p>
                <div className="flex items-center justify-between text-sm">
                  <span className="bg-yellow-400 text-black font-bold px-2 py-1 rounded">90-130€</span>
                  <span className="text-gray-500">60 min</span>
                </div>
              </Link>
              <Link href="/taxi-palaiseau" className="bg-gray-50 rounded-xl p-6 border border-gray-200 hover:border-yellow-400 transition-colors block">
                <div className="flex items-center gap-2 mb-2 text-yellow-600">
                  <MapPin className="w-5 h-5" />
                  <span className="text-sm font-medium">Ville</span>
                </div>
                <h3 className="font-bold text-gray-900 text-lg mb-2">Taxi {FROM}</h3>
                <p className="text-gray-600 text-sm mb-3">Toutes nos prestations à {FROM} : gares, aéroports, transport médical, entreprises de Saclay.</p>
